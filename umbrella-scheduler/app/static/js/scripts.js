...
// check db for conflicting time ranges
...
function validateForm() {
    ...
    let targ = "";
    $("input:checkbox[name=targets]:checked").each(function(){
        targ += `,${$(this).val()}`;
    });
    let targets = targ.substring(1);
    console.log(targets)
        ...
        while(1) {
            ...
            if (startVal && endVal) {
                ...
                if (startHour > endHour) {
                    ...
                    else if (checker['value'] == 'target') {
                        alert(`Please ensure a target is selected`);
                        return false;
                    }
                    else {
        ...
        for (day of days) {
            try {
                ...
                if (Object.keys(data[day]).length > 0) {
                    let targets;
                    targets = data[day][0]['targets'];
                    console.log(targets);
                    let target;
                    for (target of targets.split(",")) {
